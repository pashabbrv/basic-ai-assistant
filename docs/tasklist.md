...
|----------|--------|----------|
| 1. Бот с /start и LLM | ⏳ Не начато | /start → приветствие; сообщение → ответ от модели |
...
- [ ] Структура проекта по vision (bot/, main.py, pyproject.toml)
- [ ] Зависимости: aiogram, openai, uv
- [ ] Точка входа: запуск бота, Long Polling
- [ ] config.py: минимум TELEGRAM_TOKEN (остальное можно хардкод на шаг)
- [ ] Хэндлер /start: приветствие, пустая история для user_id
- [ ] Хранилище истории в памяти: dict[user_id, list]
- [ ] llm.py: ask_llm(history, message) → str, Together AI (openai SDK + base_url)
- [ ] Хэндлер текста: сообщение в историю → ask_llm (system_prompt + история) → ответ в историю → отправить пользователю
